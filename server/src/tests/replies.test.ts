	...
	describe("Test fetchReplies function which fetches relevant replies from the database and sorts them in chronological order.", async () => {
		test("Test different cheet IDs.", async () => {
			...
			expect([
				...
				replies1[0].text,
				replies1[0].cheetId,
			]).toEqual(["testuser1", "test reply 1", 1]);
			...
			expect([
				...
				replies2[0].text,
				replies2[0].cheetId,
			]).toEqual(["testuser1", "test reply 3", 2]);
			expect([
				replies2[1].username,
				replies2[1].text,
				replies2[1].cheetId,
			]).toEqual(["testuser1", "test reply 4", 2]);
			expect([
				replies2[2].username,
				replies2[2].text,
				replies2[2].cheetId,
			]).toEqual(["testuser2", "test reply 6", 2]);
			expect([
				replies2[3].username,
				replies2[3].text,
				replies2[3].cheetId,
			]).toEqual(["testuser2", "test reply 8", 2]);
			...
			expect([
				...
				replies3[0].text,
				replies3[0].cheetId,
			]).toEqual(["testuser2", "test reply 5", 3]);
			expect([
				replies3[1].username,
				replies3[1].text,
				replies3[1].cheetId,
			]).toEqual(["testuser2", "test reply 7", 3]);
			expect([
				replies3[2].username,
				replies3[2].text,
				replies3[2].cheetId,
			]).toEqual(["testuser2", "test reply 9", 3]);
			...
			expect([
				...
				replies5[0].text,
				replies5[0].cheetId,
			]).toEqual(["testuser1", "test reply 2", 5]);
			expect([
				replies5[1].username,
				replies5[1].text,
				replies5[1].cheetId,
			]).toEqual(["testuser2", "test reply 10", 5]);
		});
		...
		describe("Post a new reply at route: [POST] /replies.", async () => {
			test("Responds with HTTP status 201 and all relevant replies when a new reply is created.", async () => {
				...
				expect([body[1].username, body[1].text]).toEqual([
					"testuser1",
					"new test reply",
				]);
			});
			test("Responds with HTTP status 400 if reply validation fails - text too short.", async () => {
				...
				expect(status).toEqual(400);
				expect(text).toEqual(
					"Reply too short - must be between 5 and 50 characters."
				);
			});
		...
		describe("Post a new reply at route: [PUT] /replies.", async () => {
			test("Responds with HTTP status 200 and all relevant replies when a reply is updated.", async () => {
				...
				expect([body[0].username, body[0].text]).toEqual([
					"testuser1",
					"test reply 1 - updated",
				]);
			});
			test("Responds with HTTP status 400 if reply validation fails - text too short.", async () => {
				const { status, text } = await request(sessionApp)
			...
			test("Responds with HTTP status 400 if reply validation fails - text parameter missing.", async () => {
				const { status, text } = await request(sessionApp).put(
					"/cheets/1/replies/1"
				);
				expect(status).toEqual(400);
				expect(text).toEqual("Text not provided.");
		...
		describe("Post a new reply at route: [DELETE] /replies.", async () => {
