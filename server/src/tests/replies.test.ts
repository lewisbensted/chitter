    ...
    describe("Test fetchReplies function which fetches relevant replies from the database and sorts them in chronological order.", async () => {
        ...
        describe("Fetch replies at route: [GET] /replies.", async () => {
            ...
            test("Responds with HTTP status 400 when an invalid cheet ID is provided.", async () => {
                const { status, text } = await request(sessionApp).get("/cheets/1a/replies");
            ...
            test("Responds with HTTP status 404 if reply's target cheet does not exist in the database.", async () => {
                const { status, text } = await request(sessionApp).get("/cheets/6/replies");
        ...
        describe("Post a new reply at route: [POST] /replies.", async () => {
            ...
            test("Responds with HTTP status 400 if reply validation fails - text too short.", async () => {
                const { status, text } = await request(sessionApp).post("/cheets/1/replies").send({ text: "test" });
                expect(status).toEqual(400);
                expect(text).toEqual("Reply too short - must be between 5 and 50 characters.");
            });
            test("Responds with HTTP status 400 if reply validation fails - no text field.", async () => {
                const { status, text } = await request(sessionApp).post("/cheets/1/replies");
                expect(status).toEqual(400);
        ...
        describe("Updates an existing reply at route: [PUT] /replies.", async () => {
            ...
            test("Responds with HTTP status 400 if reply validation fails - text too short.", async () => {
                const { status, text } = await request(sessionApp).put("/cheets/1/replies/1").send({ text: "test" });
                expect(status).toEqual(400);
                expect(text).toEqual("Reply too short - must be between 5 and 50 characters.");
            });
            test("Responds with HTTP status 400 if reply validation fails - text parameter missing.", async () => {
                const { status, text } = await request(sessionApp).put("/cheets/1/replies/1");
                expect(status).toEqual(400);
                expect(text).toEqual("Text not provided.");
            });
            test("Responds with HTTP status 400 when an invalid cheet ID is provided.", async () => {
            ...
            test("Responds with HTTP status 400 when an invalid reply ID is provided.", async () => {
                const { status, text } = await request(sessionApp)
                    ...
                    .send({ text: "test reply 1 - updated" });
                expect(status).toEqual(400);
                expect(text).toEqual("Invalid reply ID provided - must be a number.");
            });
            test("Responds with HTTP status 403 if reply's userID does not match the session's userID (trying to update someone else's cheet).", async () => {
                ...
                expect(text).toEqual("Cannot update someone else's reply.");
            });
            test("Responds with HTTP status 404 if the reply to be updated does not exist in the database.", async () => {
                const { status, text } = await request(sessionApp)
                    ...
                    .send({ text: "test reply 1 - updated" });
                expect(status).toEqual(404);
                expect(text).toEqual("No Reply found with ID provided.");
            });
            test("Responds with HTTP status 404 if reply's target cheet does not exist in the database.", async () => {
                const { status, text } = await request(sessionApp)
            ...
            test("Responds with HTTP status 404 if reply's cheet ID does not match the cheet ID provided.", async () => {
                const { status, text } = await request(sessionApp).put("/cheets/2/replies/1");
                expect(status).toEqual(403);
        ...
        describe("Deletes an existing reply at route: [DELETE] /replies.", async () => {
            ...
            test("Responds with HTTP status 400 when an invalid cheet ID is provided.", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/1a/replies/1");
                expect(status).toEqual(400);
                expect(text).toEqual("Invalid cheet ID provided - must be a number.");
            });
            test("Responds with HTTP status 400 when an invalid reply ID is provided.", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/1/replies/1a");
                expect(status).toEqual(400);
                expect(text).toEqual("Invalid reply ID provided - must be a number.");
            });
            test("Responds with HTTP status 403 if reply's userID does not match the session's userID (trying to update someone else's reply).", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/2/replies/6");
                expect(status).toEqual(403);
                expect(text).toEqual("Cannot delete someone else's reply.");
            });
            test("Responds with HTTP status 404 if the reply to be updated does not exist in the database.", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/1/replies/11");
                expect(status).toEqual(404);
                expect(text).toEqual("No Reply found with ID provided.");
            });
            test("Responds with HTTP status 404 if reply's target cheet does not exist in the database.", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/6/replies/1");
                expect(status).toEqual(404);
                expect(text).toEqual("No Cheet found with ID provided.");
            });
            test("Responds with HTTP status 404 if reply's cheet ID does not match the cheet ID provided.", async () => {
                const { status, text } = await request(sessionApp).delete("/cheets/2/replies/1");
                expect(status).toEqual(403);
                expect(text).toEqual("Cheet IDs do not match.");
            });
        });
