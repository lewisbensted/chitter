...
export interface ExtendedUserClient {
	...
	findUnique(
		args: Parameters<PrismaClient["user"]["findUnique"]>[0]
	): Promise<(IUser & { passwordHash: string }) | null>;
	create(args: { data: RegisterUserRequestBody }): Promise<IUser>;
	createMany(args: Parameters<PrismaClient["user"]["createMany"]>[0]): Promise<IUser[]>;
}
...
export interface ExtendedCheetClient {
	...
	createMany(args: Parameters<PrismaClient["cheet"]["createMany"]>[0]): Promise<ICheet[]>;
...
export interface ExtendedReplyClient {
	...
	createMany(args: Parameters<PrismaClient["reply"]["createMany"]>[0]): Promise<IReply[]>;
...
export interface ExtendedMessageClient {
	...
	createMany(args: Parameters<PrismaClient["message"]["createMany"]>[0]): Promise<IMessage[]>;
}
