...
export type SendCheetRequest = Request<Record<string, never>, ICheet, { text: string; uuid?: string }> & {
...
export type SendReplyRequest = Request<{ cheetId: string }, IReply, { text: string; uuid?: string }> & {
	session: { user?: { uuid: string } };
};
...
export type SendMessageRequest = Request<{ recipientId: string }, IMessage, { text: string; uuid?: string }> & {
	session: { user?: { uuid: string } };
};
